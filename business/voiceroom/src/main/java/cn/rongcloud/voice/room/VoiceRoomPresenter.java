...
//        }
        if (currentStatus == STATUS_ON_SEAT) {
            if (-1 == position) {
                UiSeatModel selfModel = voiceRoomModel.getSeatInfoByUserId(UserManager.get().getUserId());
                if (null != selfModel) {
                    showNewSelfSettingFragment(selfModel).show(((VoiceRoomFragment) mView).getChildFragmentManager());
                }
            }
            return;
        }
    ...
    private void setObRoomEventChange() {
        disposableList.add(voiceRoomModel.obRoomEventChange().subscribe(new Consumer<Pair<String, ArrayList<String>>>() {
            ...
            public void accept(Pair<String, ArrayList<String>> stringArrayListPair) throws Throwable {
                Logger.d(TAG, "obRoomEventChange: pair = " + GsonUtil.obj2Json(stringArrayListPair));
                String first = stringArrayListPair.first;
                List<String> list = stringArrayListPair.second;
                try {
                    if (TextUtils.equals(first, EVENT_ADD_SHIELD)) {
                        ...
                        if (null != list && !list.isEmpty() && TextUtils.equals(list.get(0), UserManager.get().getUserId())) {
                            KToast.show("用户连线成功");
                        }
            ...
            @Override
            public void accept(Throwable throwable) throws Throwable {
                Logger.e(TAG, "setObRoomEventChange: " + throwable);
            }
        }));
    }
