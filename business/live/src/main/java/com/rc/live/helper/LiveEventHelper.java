...
import cn.rongcloud.thirdcdn.ThirdCDNPlayer;
...
public class LiveEventHelper implements ILiveEventHelper, RCLiveEventListener, RCLiveLinkListener, RCLiveSeatListener, RCLivePKListener, OnCloseMiniRoomListener {
    ...
    public void setLiveType(RCLiveInfo.RCLiveType liveType) {
    ...
    public void register(String roomId) {
        ...
        // 启用消息同步用户进出房间事件
        RCLiveEngine.getInstance().enableInOutRoomEvent(true);
    ...
    public void unRegister() {
        ...
        liveType = RCLiveInfo.RCLiveType.INNER_CDN;
    ...
    public void joinRoom(String roomId, ClickCallback<Boolean> callback) {
        register(roomId);
        RCLiveInfo liveInfo = RCLiveInfo.Builder
                .create(liveType)
                .setThirdCDNPlayer(new ThirdCDNPlayer(UIKit.getContext()))
                .build();
        RCLiveEngine.getInstance().joinRoom(roomId, liveInfo, new RCLiveCallback() {
            @Override
            public void onSuccess() {
    ...
    public void begin(String roomId, ClickCallback<Boolean> callback) {
        register(roomId);
        String url = ThirdCDNConstant.getPushUrl(roomId);
        RCLiveEngine.getInstance().begin(roomId, url, new RCLiveCallback() {
            @Override
            public void onSuccess() {
                //开启直播并且加入房间成功
                Log.e(TAG, "onSuccess: ");
    ...
    @Override
    public void onUserUpdate(List<String> joins, List<String> exits) {
        for (LiveRoomListener liveRoomListener : liveRoomListeners) {
            liveRoomListener.onUserUpdate(joins, exits);
        }
    }
