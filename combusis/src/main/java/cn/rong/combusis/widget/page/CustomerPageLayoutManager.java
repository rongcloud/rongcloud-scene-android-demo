...
package cn.rong.combusis.widget.page;

import static android.view.View.MeasureSpec.EXACTLY;
import static cn.rong.combusis.widget.page.PagerConfig.Loge;
import static cn.rong.combusis.widget.page.PagerConfig.Logi;
...
public class CustomerPageLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    public static final int VERTICAL = 0;           // 垂直滚动
    public static final int HORIZONTAL = 1;         // 水平滚动
    private static final String TAG = CustomerPageLayoutManager.class.getName();
     ...
     */
    public CustomerPageLayoutManager(@IntRange(from = 1, to = 100) int rows,
...
//        }
     ...
     */
    @Override
    public void onMeasure(RecyclerView.Recycler recycler, @NotNull RecyclerView.State state, int widthMeasureSpec, int heightMeasureSpec) {
    ...
    public View findSnapView() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            int childPos = getPosition(Objects.requireNonNull(getChildAt(i)));
    ...
    public int setOrientationType(@OrientationType int orientation) {
        if (mOrientation == orientation || mScrollState != RecyclerView.SCROLL_STATE_IDLE)
            return mOrientation;
        mOrientation = orientation;
        mItemFrames.clear();
        int x = mOffsetX;
