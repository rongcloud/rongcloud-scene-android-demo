...
public class BeautyManager {
    private static final String BUNDLE_AI_HAND = "model" + File.separator + "ai_hand_processor.bundle";

    // 注册美颜
    public static int register(byte[] v3data, byte[] authData) {
        int result = RCRTCFUBeautifierEngine.getInstance().register(null, authData);
        if (result == 1) {
            // rtc 插件内部没有添加手势bundle,这里添加一下
            FURenderKit.getInstance().getFUAIController().loadAIProcessor(BUNDLE_AI_HAND, FUAITypeEnum.FUAITYPE_HANDGESTURE);
        }
        return result;
    }
