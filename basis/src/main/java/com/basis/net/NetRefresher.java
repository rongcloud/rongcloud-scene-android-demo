 ...
 *
 */
public abstract class NetRefresher<T> implements BusiCallback<IResult.WrapResult<List<T>, IPage>, List<T>, IPage, T> {
...
//        ORequest = Request.requestForPage(dialog, url, params, parser, method, this);
     ...
     * @param total
     */
    abstract void onEnableLoadMore(int total);

    @Override
    public void onResult(IResult.WrapResult<List<T>, IPage> result) {
        if (null != operator) {
            onRefreshData(result.getResult(), refresh);//注意 此处不是使用的isRefresh
        }
        IPage page = result.getExtra();
        onEnableLoadMore(page.getTotal());
    }
    ...
    protected void onRefreshData(List<T> data, boolean refresh) {
